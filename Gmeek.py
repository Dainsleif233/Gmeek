...
class GMEEK:
    ...
    def defaultConfig(self):
        ...
        if "primerCSS" not in self.blogBase:
            self.blogBase["primerCSS"] = (
                "<link href='https://mirrors.sustech.edu.cn/cdnjs/ajax/libs/Primer/21.0.7/primer.css' rel='stylesheet' />"
            )

        if "homeUrl" not in self.blogBase:
            if (
                str(self.repo.name).lower()
                == (str(self.repo.owner.login) + ".github.io").lower()
            ):
                self.blogBase["homeUrl"] = f"https://{self.repo.name}"
            else:
                self.blogBase["homeUrl"] = (
                    f"https://{self.repo.owner.login}.github.io/{self.repo.name}"
                )
    ...
    def renderHtml(self, template, blogBase, postListJson, htmlDir, icon):
        ...
        output = template.render(
            blogBase=blogBase, postListJson=postListJson, i18n=self.i18n, IconList=icon
        )
    ...
    def createPostHtml(self, issue):
        ...
        print(
            "create postPage title=%s file=%s " % (issue["postTitle"], issue["htmlDir"])
        )
    ...
    def createPlistHtml(self):
        ...
        while True:
            ...
            if postNum <= self.blogBase["onePageListNum"]:
                if pageFlag == 0:
                    onePageList = dict(
                        list(self.blogBase["postListJson"].items())[:postNum]
                    )
                    ...
                    onePageList = dict(
                        list(self.blogBase["postListJson"].items())[
                            topNum : topNum + postNum
                        ]
                    )
                ...
                self.renderHtml(
                    "plist.html", self.blogBase, onePageList, htmlDir, plistIcon
                )
                ...
                onePageList = dict(
                    list(self.blogBase["postListJson"].items())[
                        topNum : topNum + self.blogBase["onePageListNum"]
                    ]
                )
        ...
        self.renderHtml(
            "tag.html", self.blogBase, onePageList, self.root_dir + "tag.html", tagIcon
        )
    ...
    def addOnePostJson(self, issue):
        if len(issue.labels) >= 1:
            ...
            self.blogBase[listJsonName][postNum]["labels"] = [
            ...
            if "timestamp" in postConfig:
                self.blogBase[listJsonName][postNum]["createdAt"] = postConfig[
                    "timestamp"
                ]
            else:
            ...
            if "style" in postConfig:
                self.blogBase[listJsonName][postNum]["style"] = self.blogBase[
                    "style"
                ] + str(postConfig["style"])
            else:
                self.blogBase[listJsonName][postNum]["style"] = self.blogBase["style"]

            if "script" in postConfig:
                self.blogBase[listJsonName][postNum]["script"] = self.blogBase[
                    "script"
                ] + str(postConfig["script"])
            else:
                self.blogBase[listJsonName][postNum]["script"] = self.blogBase["script"]

            if "head" in postConfig:
                self.blogBase[listJsonName][postNum]["head"] = self.blogBase[
                    "head"
                ] + str(postConfig["head"])
            else:
            ...
            if "ogImage" in postConfig:
                ...
                self.blogBase[listJsonName][postNum]["ogImage"] = self.blogBase[
                    "ogImage"
                ]
            ...
            self.blogBase[listJsonName][postNum]["createdDate"] = thisTime.strftime(
                "%Y-%m-%d"
            )
            self.blogBase[listJsonName][postNum]["dateLabelColor"] = self.blogBase[
                "yearColorList"
    ...
    def createFileName(self, issue, useLabel=False):
        if useLabel == True:
            ...
            if self.blogBase["urlMode"] == "issue":
                ...
                fileName = str(
                    translit(issue.title, language_code="ru", reversed=True)
                ).replace(" ", "-")
            else:
...
blog.blogBase["postListJson"] = dict(
    sorted(
...
if os.environ.get("GITHUB_EVENT_NAME") != "schedule":
    ...
    readme = readme + "### :page_facing_up: [%d](%s/tag.html) \r\n" % (
        len(blog.blogBase["postListJson"]) - 1,
    ...
    readme = (
        readme + "### Powered by :heart: [Gmeek](https://github.com/Meekdai/Gmeek)\r\n"
    )
